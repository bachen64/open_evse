#if defined(ENABLE_DEBUG) && !defined(ENABLE_DEBUG_J1772EVSECONTROLLER)
#undef ENABLE_DEBUG
#endif
...
//                                               A/B B/C C/D D DS
THRESH_DATA J1772EVSEController::m_ThreshData = {
#if defined(ESP32)
    // ADC12BIT
    3849, // m_ThreshAB <= phigh    4095
    3276, // m_ThreshBC <= phigh    3561
    2457, // m_ThreshCD <= phigh    2867
    2293, // m_ThreshD  <= phigh    2382
    //2129, // m_ThreshPE <= plow as last condition
    1065  // m_ThreshDS <= plow     2064
#elif defined(__LGT8F__)
    // ADC12BIT, DEFAULT ----> 5000 mV / 4095 unit = 1,221001221 mV/unit
    3522, // State A -> B   4300mV ----> mV = unit * 5000/4095
    3112, // State B -> C   3800mV
    2703, // State C -> D   3300mV
    2375, // Vent Required  2900mV
    //1966, // Pilot Error    2400mV
    1065  // Diode Short    1300mV
#elif defined(__AVR__)
    // ADC10BIT, DEFAULT ----> 5000 mV / 1023 value = 4,887585533 mV/unit
     880, // State A -> B   4300mV ----> unit = mV * 1023/5000
     777, // State B -> C   3800mV
     675, // State C -> D   3300mV
     593, // Vent Required  2900mV
    // 491, // Pilot Error    2400mV
     266  // Diode Short    1300mV ----> mV = unit * 5000/1023
#else
     875, // m_ThreshAB
     780, // m_ThreshBC
     690, // m_ThreshCD
     590, // m_ThreshD      original = 0
    // 490, // m_ThreshPE
     260  // m_ThreshDS
#endif
};
...
void J1772EVSEController::readAmmeter()
{
  ...
  {
    // the A/d is 0 to 1023(ADC10BIT) or 4095(ADC12BIT).
    ...
    if (!is_first_sample && (
#if defined(ESP32) || defined(__LGT8F__)
                                (last_sample > 2048) != (sample > 2048)
#elif defined(__AVR__)
                                (last_sample > 512) != (sample > 512)
#else
                                (last_sample > 512) != (sample > 512)
#endif
                                    ))
    ...
    case 2:
      ...
      sum += (unsigned long)(
#if defined(ESP32) || defined(__LGT8F__)
          ((long)sample - 2048) * ((long)sample - 2048)
#elif defined(__AVR__)
          ((long)sample - 512) * ((long)sample - 512)
#else
          ((long)sample - 512) * ((long)sample - 512)
#endif
      );
...
J1772EVSEController::J1772EVSEController() : adcPilot(PILOT_PIN)
#ifdef CURRENT_PIN
                                             ,
...
#ifdef RELAY_PWM
    Serial.print("\nrelayCloseMs: ");
    Serial.println(m_relayCloseMs);
    Serial.print("relayHoldPwm: ");
    Serial.println(m_relayHoldPwm);
...
void J1772EVSEController::HardFault(int8_t recoverable)
{
  ...
  RapiSendEvseState();
#endif
// NOTE: moved into open_evse_loop()
#ifndef OPEN_EVSE_LIB
  while (1)
  {
...
void J1772EVSEController::SetGfiTripped()
{
  ...
  {
    if (!m_Gfi.SelfTestSuccess()) m_Gfi.SetTestSuccess();
    return;
  }
#endif
...
uint8_t J1772EVSEController::EnableAutoSvcLevel(uint8_t tf)
{
  ...
  if (tf)
  {
    if (CGMIisEnabled())
      ...
      clrFlags(ECF_AUTO_SVC_LEVEL_DISABLED);
  }
  else
  {
...
void J1772EVSEController::SetSvcLevel(uint8_t svclvl, uint8_t updatelcd)
{
  ...
  if (SerDbgEnabled())
  {
    Serial.print("SetSvcLevel: ");
    Serial.println((int)svclvl);
  }
#endif // #ifdef SERDBG
  DBUGF("SetSvcLevel: %d", (int)svclvl);
  if (svclvl == 2)
  {
  ...
  {
    svclvl = 1;          // force invalid value to SL1
...
uint8_t J1772EVSEController::doPost()
{
  ...
  {
#ifdef OPENEVSE_2
    // For OpenEVSE II, there is a voltmeter for auto SL1/SL2.
    ...
    if (SerDbgEnabled())
    {
      Serial.print("AC millivolts: ");
      Serial.println(ac_volts);
      Serial.print("SvcState: ");
      Serial.println((int)svcState);
    }
#endif // #ifdef SERDBG
    DBUGF("AC millivolts: %d", ac_volts);
    DBUGF("SvcState: %d", (int)svcState);
#ifdef LCD16X2
    g_OBD.LcdMsg_P(g_psAutoDetect, (svcState == SL2) ? g_psLevel2 : g_psLevel1);
#endif // LCD16x2
...
#endif // #ifdef SERDBG
    DBUGF("Pilot: %d", (int)reading);

    m_Pilot.SetState(PILOT_STATE_N12);
    if (reading >= m_ThreshData.m_ThreshAB)
    { // IF EV is not connected its Okay to open the relay the do the SL1/SL2 and ground Check
      ...
      // decide input power state based on the status read  on SL1 and SL2
      // either 2 SPST or 1 DPST relays can be configured
      // valid svcState is SL1 - one hot, SL2 both hot, OG - open ground both off, SR - stuck relay when shld be off
      //
      if (RelayOff == noneon)
      { // relay not stuck on when off
        switch (Relay1)
        {
        case (both): //
          if (Relay2 == noneon)
        ...
        case (noneon): //
          ...
          if (Relay2 == both)
            svcState = SL2;
          if (Relay2 == SL1 || Relay2 == SL2)
            svcState = SL1;
          break;
        case (L1on): // SL1 or SL2
        case (L2on):
          if (StuckRelayChkEnabled())
          {
            if (Relay2 != noneon)
          ...
          if (Relay2 == noneon)
            svcState = SL1;
          if ((Relay1 == L1on) && (Relay2 == L2on))
            svcState = SL2;
          if ((Relay1 == L2on) && (Relay2 == L1on))
            svcState = SL2;
          break;
        } // end switch
      }
      ...
      if (SerDbgEnabled())
      {
        ...
        Serial.println((int)RelayOff);
        Serial.print("Relay1: ");
        Serial.println((int)Relay1);
        Serial.print("Relay2: ");
        Serial.println((int)Relay2);
        Serial.print("SvcState: ");
        Serial.println((int)svcState);
      }
#endif // #ifdef SERDBG
      DBUGF("RelayOff: %d", (int)RelayOff);
      DBUGF("Relay1: %d", (int)Relay1);
      DBUGF("Relay2: %d", (int)Relay2);
      DBUGF("SvcState: %d", (int)SvcState);
      ...
      if (svcState == OG)
        g_OBD.LcdMsg_P(g_psTestFailed, g_psNoGround);
      if (svcState == SR)
    ...
    {
      // since we can't auto detect, for safety's sake, we must set to SL1
      svcState = SL1;
  ...
  if (((svcState == UD) || (svcState == SL1) || (svcState == SL2)) &&
    GfiSelfTestEnabled() && m_Gfi.SelfTest())
  {
#ifdef LCD16X2
...
#endif // #ifdef SERDBG
  DBUGF("POST result: %d", (int)svcState);
...
void J1772EVSEController::Init()
{
  ...
  if (DPIN_READ(V6_ID_REG, V6_ID_IDX))
    m_isV6 = 0;
  else
    m_isV6 = 1;
#else
...
#else
  pinChargingAC.init(NULL, CHARGING, DigitalPin::OUT);
#endif
#endif
#ifdef ACLINE1_REG
#ifndef OPEN_EVSE_LIB
...
#else
  pinAC1.init(NULL, ESP_AT1, DigitalPin::INP_PU);
#endif
#endif
#ifdef ACLINE2_REG
#ifndef OPEN_EVSE_LIB
...
#else
  pinAC2.init(NULL, ESP_AT2, DigitalPin::INP_PU);
#endif
#endif
#ifdef SLEEP_STATUS_REG
...
#ifdef AUTOSVCLEVEL
    if ((AutoSvcLevelEnabled()) && ((psvclvl == SL1) || (psvclvl == SL2)))
      svclvl = psvclvl; // set service level
#endif                  // AUTOSVCLEVEL
  ...
  { // EEPROM not initialized, let's just load the default values eh?  <== CANADA
...
{
#if defined(ESP32) || defined(__LGT8F__)
  uint16_t pl = 4095; // ADC12BIT
#elif defined(__AVR__)
  uint16_t pl = 1023; // ADC10BIT
#endif
  uint16_t ph = 0;

  // #define PILOT_READ  36  // INP     ADC1_CHANNEL_0
...
#ifdef MENNEKES_LOCK
      if (!MennekesIsManual())
        m_MennekesLock.Unlock(0);
#endif // MENNEKES_LOCK
    }
    else if (ph > 0 && pl > 0 && ph > pl) // BAC: plausibility check added
    {
      SetEvConnected();
#ifdef MENNEKES_LOCK
      if (!MennekesIsManual())
        m_MennekesLock.Lock(0);
#endif // MENNEKES_LOCK
    }
  }
...
// Negative Voltage, States B/C/D/F -11.40    -12.00    -12.60
void J1772EVSEController::Update(uint8_t forcetransition)
{
...
#if defined(AMMETER) && defined(CURRENT_PIN)
      readAmmeter();
  ...
  {
    DBUGLN("CGMI: AT2 relay didn't close");
    // bad ground
  ...
  if (nofault)
  {
    ...
    { // relay closed
      ...
      {
        DBUGLN("CGMI: AT1 relay didn't close");
        chargingOff(); // open the relay
      ...
      else if (!CGMIisEnabled() && ((curms - m_ChargeOnTimeMS) > GROUND_CHK_DELAY))
      {
        ...
        {
          DBUGLN("AT1 and AT2 both relays didn't close");
          // bad ground
          tmpevsestate = EVSE_STATE_NO_GROUND;
          m_EvseState = EVSE_STATE_NO_GROUND;
        ...
        // so we had to hardcode SL1. During first charge session, we can probe and set to SL2 if necessary
    ...
    { // !chargingIsOn() - relay open
      ...
      { // stuck relay check - can test only when relay open
        if ((CGMIisEnabled() && !(acpinstate & RLY_TEST_PIN_OPEN)) ||
            (!CGMIisEnabled() && (acpinstate != ACPINS_OPEN)))
        { // Stuck Relay reading
          if ((prevevsestate != EVSE_STATE_STUCK_RELAY) && !m_StuckRelayStartTimeMS)
          ...
          m_StuckRelayStartTimeMS = 0; // not stuck - reset
  ...
  if (m_Gfi.Fault())
  {
    ...
    else // prevevsestate == EVSE_STATE_GFCI_FAULT
    {
      if (!EvConnected())
      {
        DBUGLN("EV disconnected - cancel GFCI fault");
        m_EvseState = EVSE_STATE_UNKNOWN;
        m_Gfi.Reset();
        return;
      }
      // EV connected, retry in xx:xx s
      else if ((curms - m_GfiFaultStartMs) >= GFI_TIMEOUT)
      {
#ifdef FT_GFI_RETRY
        ...
        {
          DBUGF("%d times of retry test, giving up", m_GfiRetryCnt);
          HardFault(1);
          return;
        }
        else
        {
          DBUGF("%d. time to retry test before giving up", m_GfiRetryCnt);
          m_Gfi.Reset();
          m_GfiFaultStartMs = 0;
        }
      }
  ...
  if (nofault)
  {
    ...
    {
      DBUGLN("just got out of fault state - pilot back on");
      m_Pilot.SetState(PILOT_STATE_P12);
    ...
    if (DiodeCheckEnabled()
    && (m_Pilot.GetState() == PILOT_STATE_PWM || phigh <= m_ThreshData.m_ThreshD)
    && (plow >= m_ThreshData.m_ThreshDS)
    ) {
      // diode check failed
    ...
    // evse and pilot states
    // EVSE STATE
    //    prevevsestate = m_EvseState;    // save
    //    tmpevsestate = xxx;             // reset
    //    m_EvseState = tmpevsestate;     // reset
    // PILOT STATE
    //    prevpilotstate = m_PilotState;  // save
    //    tmpevsestate = xxx;             // reset
    //    tmppilotstate = xxx;            // reset
    // FT ENDURANCE
    //    tmpevsestate = xxx;             // reset
    ...
    if (tmpevsestate != prevevsestate)
    {
      ...
      else // confirmed tmpevsestate == m_TmpEvseState
      if ((curms - m_TmpEvseStateStart) >= ((tmpevsestate == EVSE_STATE_A) ? DELAY_STATE_TRANSITION_A : DELAY_STATE_TRANSITION))
      {
  ...
  if (tmppilotstate != prevpilotstate)
  {
    ...
    else // confirmed tmppilotstate == m_TmpPilotState
    if ((curms - m_TmpPilotStateStart) >= DELAY_STATE_TRANSITION)
    {
  ...
  if (forcetransition || (m_EvseState != prevevsestate))
  {
    ...
    else if (m_EvseState == EVSE_STATE_B)
    {                // connected
      chargingOff(); // turn off charging current
#ifdef AUTH_LOCK
    ...
    else if (m_EvseState == EVSE_STATE_C)
    {
      ...
      if (GfiSelfTestEnabled() && m_Gfi.SelfTest()) {
        DBUGLN("GFI test failed before closing relay");
        m_EvseState = EVSE_STATE_GFI_TEST_FAILED;
        m_Pilot.SetState(PILOT_STATE_P12);
        HardFault(1);
        m_Gfi.Reset(); // try to reset GFI
        return;
      }
    ...
    {
      DBUGLN("vent required not supported");
      chargingOff(); // turn off charging current
      m_Pilot.SetState(PILOT_STATE_P12);
      HardFault(1);
    }
    ...
    {
      DBUGLN("vehicle state Over Teperature within the EVSE");
    ...
    {
      DBUGLN("Ground missing detected");
      chargingOff(); // turn off charging current
      m_Pilot.SetState(PILOT_STATE_P12);
    }
    else if (m_EvseState == EVSE_STATE_STUCK_RELAY)
    {
      DBUGLN("Stuck relay detected");
      chargingOff(); // turn off charging current
#ifdef UL_COMPLIANT
...
#endif // #ifdef SERDBG
    DBUGF("state %s->%s %s p %d %d",
      prevevsestate == EVSE_STATE_A ? "A" :
      prevevsestate == EVSE_STATE_B ? "B" :
      prevevsestate == EVSE_STATE_C ? "C" :
      prevevsestate == EVSE_STATE_D ? "D" :
      prevevsestate == EVSE_STATE_DIODE_CHK_FAILED ? "DIODE" :
      prevevsestate == EVSE_STATE_GFCI_FAULT ? "F" :
      prevevsestate == EVSE_STATE_NO_GROUND ? "NOGND" :
      prevevsestate == EVSE_STATE_STUCK_RELAY ? "STUCK" :
      prevevsestate == EVSE_STATE_GFI_TEST_FAILED ? "TEST" :
      prevevsestate == EVSE_STATE_OVER_TEMPERATURE ? "OT" :
      prevevsestate == EVSE_STATE_OVER_CURRENT ? "OC" :
      prevevsestate == EVSE_STATE_RELAY_CLOSURE_FAULT ? "RC" :
      prevevsestate == EVSE_STATE_SLEEPING ? "SLEEPING" :
      prevevsestate == EVSE_STATE_DISABLED ? "DISABLED" : "?",
      m_EvseState == EVSE_STATE_A ? "A" :
      m_EvseState == EVSE_STATE_B ? "B" :
      m_EvseState == EVSE_STATE_C ? "C" :
      m_EvseState == EVSE_STATE_D ? "D" :
      m_EvseState == EVSE_STATE_DIODE_CHK_FAILED ? "DIODE" :
      m_EvseState == EVSE_STATE_GFCI_FAULT ? "F" :
      m_EvseState == EVSE_STATE_NO_GROUND ? "NOGND" :
      m_EvseState == EVSE_STATE_STUCK_RELAY ? "STUCK" :
      m_EvseState == EVSE_STATE_GFI_TEST_FAILED ? "TEST" :
      m_EvseState == EVSE_STATE_OVER_TEMPERATURE ? "OT" :
      m_EvseState == EVSE_STATE_OVER_CURRENT ? "OC" :
      m_EvseState == EVSE_STATE_RELAY_CLOSURE_FAULT ? "RC" :
      m_EvseState == EVSE_STATE_SLEEPING ? "SLEEPING" :
      m_EvseState == EVSE_STATE_DISABLED ? "DISABLED" : "?",
      m_Pilot.GetState() == PILOT_STATE_P12 ? "P12" :
      m_Pilot.GetState() == PILOT_STATE_PWM ? "PWM" :
      m_Pilot.GetState() == PILOT_STATE_N12 ? "N12" : "ERR",
      plow, phigh
    );
  } // state transition
  ...
  if (!nofault && (prevevsestate == EVSE_STATE_C))
  {
    ...
    {
      DBUGLN("fault in charging - immediately hard fault");
      HardFault(1);
      return;
    }
  }
  ...
  if (m_EvseState == EVSE_STATE_C)
  {
    ...
    {
      if (m_OverCurrentStartMs)
      { // already in overcurrent state
  ...
  if (TempChkEnabled())
  {
    ...
    if (g_TempMonitor.OverTemperature() && ((g_TempMonitor.m_TMP007_temperature <= TEMPERATURE_INFRARED_RESTORE_AMPERAGE) && // all sensors need to show return to lower levels
                                            ...
                                            (g_TempMonitor.m_DS3231_temperature <= TEMPERATURE_AMBIENT_RESTORE_AMPERAGE)))
    {            // restore the original SL2 current advice to the EV
      setit = 1; // set to the user's original setting for current
                                                         ...
                                                         (g_TempMonitor.m_DS3231_temperature <= TEMPERATURE_AMBIENT_THROTTLE_DOWN)))
    {               //  restore the throttled down current advice to the EV since things have cooled down again
    ...
    if (TempChkEnabled())
    {
      if (m_ElapsedChargeTime != m_ElapsedChargeTimePrev)
      {
        ...
        if (!g_TempMonitor.OverTemperature() && ((g_TempMonitor.m_TMP007_temperature >= TEMPERATURE_INFRARED_THROTTLE_DOWN) || // any sensor reaching threshold trips action
                                                 (g_TempMonitor.m_MCP9808_temperature >= TEMPERATURE_AMBIENT_THROTTLE_DOWN) ||
                                                 (g_TempMonitor.m_DS3231_temperature >= TEMPERATURE_AMBIENT_THROTTLE_DOWN)))
        {               // Throttle back the SL2 current advice to the EV
          currcap /= 2; // set to the throttled back level
          setit = 2;
        }
                                                              ...
                                                              (g_TempMonitor.m_DS3231_temperature >= TEMPERATURE_AMBIENT_SHUTDOWN)))
        { // Throttle back the SL2 current advice to the EV
          currcap /= 4;
          setit = 4;
        }
...
int J1772EVSEController::HeartbeatSupervision(uint16_t interval, uint8_t amps)
{
  ...
  Serial.println(m_IFallback);
#endif
  if (eeprom_read_word((uint16_t *)EOFS_HEARTBEAT_SUPERVISION_INTERVAL) != m_HsInterval)
  { // only write EEPROM if it is needful!
#ifdef DEBUG_HS
  ...
  if (eeprom_read_byte((uint8_t *)EOFS_HEARTBEAT_SUPERVISION_CURRENT) != m_IFallback)
  { // only write EEPROM if it is needful!
#ifdef DEBUG_HS
...
#endif
  if ((m_HsTriggered == HS_MISSEDPULSE_NOACK))
  {         // We were in a state of missed pulse
    rc = 1; // We have been triggered but have not been acknowledged.  Therfore respond with NK.
  }
  else
  { // If we have been triggered it has already been dealt with (m_HsTriggered = HS_MISSEDPULSE or 0)
    rc = 0;
  }
...
int J1772EVSEController::HsRestoreAmpacity()
{
  ...
  int rc = 1;
  if (m_HsTriggered)
  { // At some point while active, HEARTBEAT_SUPERVISION was triggered, so we will have to perturb the ampacity
#ifdef DEBUG_HS
    ...
    { // We need to ensure that OverTemperature is not active before we raise current capacity
#ifdef DEBUG_HS
...
int J1772EVSEController::HsExpirationCheck()
{
  ...
  int rc = 1;
  if (m_HsInterval != 0 && sinceLastPulse > (m_HsInterval * 1000))
  { // HEARTBEAT_SUPERVISION is currently active and the Heartbeat interval has timed out
#ifdef DEBUG_HS
    Serial.println(F("HsExpirationCheck: Heartbeat timer expired account late or no pulse"));
#endif
    if (m_IFallback < GetCurrentCapacity())
    { // We in HEARTBEAT_SUPERVISION ampacity limiting but the current capacity is not OK
#ifdef DEBUG_HS
  ...
  {         // HS is ACTIVE but the timer has not yet expired and there have been no unresolved m_HsTriggered events as of yet.
...
#endif
  if (ack == HS_ACK_COOKIE)
  { // Is this a legitimate acknowlegement of missed HEARTBEAT_SUPERVISION pulse?
    if (m_HsTriggered == HS_MISSEDPULSE_NOACK)
    { // Has HEARTBEAT_SUPERVISION missed a pulse with no subsequenr acknowledgement?
#ifdef DEBUG_HS
